...
export function FreelancerRightAside() {
  ...
  const isPinnedColumn = pathname.startsWith("/freelancer-dashboard");
  ...
  return (
    <aside
      className={`${dashboardAsideClass} ${isPinnedColumn ? dashboardAsideFixedClass : "gap-8"}`}
    >
      <section className={isPinnedColumn ? "flex min-h-0 flex-1 flex-col overflow-hidden" : ""}>
        ...
        <ul
          className={`mt-3 space-y-3 ${
            isPinnedColumn ? "min-h-0 flex-1 overflow-y-auto overscroll-contain pr-0.5" : ""
          }`}
        >
